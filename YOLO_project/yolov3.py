...
# /yolov3-tensorflow-2-part-2/
def parse_cfg(cfgfile):
    # parse the YOLOv3 configuration fileyolov3.cfg.
...
def YOLOv3Net(cfgfile, model_size, num_classes):
    ...
    for i, block in enumerate(blocks[1:]):
        ...
        # Yolo detection layer
            ...
            # Refine boxes to the have the right positions and shapes.
            box_centers = tf.sigmoid(box_centers)
            confidence = tf.sigmoid(confidence)
            classes = tf.sigmoid(classes)
            anchors = tf.tile(anchors, [out_shape[1] * out_shape[2], 1])
            box_shapes = tf.exp(box_shapes) * tf.cast(anchors, dtype=tf.float32)
            # convert the relative positions of the center
            # boxes into the real positions.
            ...
            # prediction result for each scale and concatenate it with the others.
            if scale:
                out_pred = tf.concat([out_pred, prediction], axis=1)
            else:
                out_pred = prediction
                scale = 1
            # keep the track of the feature maps and output filters.
        outputs[i] = inputs
